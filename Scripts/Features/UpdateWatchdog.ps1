function Invoke-InstallUpdateWatchdog {
    ...
    try {
        ...
        # the user to re-run WinSwift. Scoped to the two controls Windows Update
...
'@
        ...
        Register-ScheduledTask -TaskName $taskName -TaskPath $taskPath -Trigger $triggers -Action $action -Principal $principal -Description "Re-asserts WinSwift telemetry settings after a Windows update resets them." | Out-Null
